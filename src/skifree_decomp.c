...
// ---- Theme support (static + live reload) ----
// The game reads only its own config file; anything that can write TOML
// (a hand editor, a script, a desktop theme template) can theme it:
//   $XDG_CONFIG_HOME/skifree/theme.toml, else ~/.config/skifree/theme.toml
// Keys are role names (snow, ink, red, green, blue, yellow, magenta, cyan);
// background/foreground are accepted as aliases. Missing file = classic look.
// Sprites are 4-bit palettized BMPs, so theming = rewriting the 16 palette
...
static ThemeRGB themeSnow = { 0xff, 0xff, 0xff };
static ThemeRGB themeInk = { 0, 0, 0 };
static ThemeRGB themeRed = { 0xff, 0, 0 };
static ThemeRGB themeGreen = { 0, 0xbf, 0 };
static ThemeRGB themeBlue = { 0, 0, 0xff };
static ThemeRGB themeYellow = { 0xff, 0xff, 0 };
static ThemeRGB themeMagenta = { 0xff, 0, 0xff };
static ThemeRGB themeCyan = { 0, 0xff, 0xff };
static ThemeRGB themeStone = { 0x80, 0x80, 0x80 };
static ThemeRGB themeMoss = { 0xbf, 0xbf, 0 };
static ThemeRGB themeBump = { 0xc0, 0xc0, 0xc0 };

#define THEME_MAX_KEYS 40
static char themeKeys[THEME_MAX_KEYS][32];
static ThemeRGB themeVals[THEME_MAX_KEYS];
static int themeKeyCount = 0;
static void recordThemeMtimes(void);
static void themeConfigPath(char* out, size_t n);
...
static int parseHexColor(const char* s, ThemeRGB* out) {
    while (*s == ' ' || *s == '\t' || *s == '"' || *s == '\'') s++;
    if (*s == '#') s++;
    int len = 0;
    ...
    if (len == 6) {
        int h[6];
        for (int i = 0; i < 6; i++) { h[i] = hexNibble(s[i]); if (h[i] < 0) return 0; }
        r = h[0] * 16 + h[1]; g = h[2] * 16 + h[3]; b = h[4] * 16 + h[5];
    } else if (len == 3) {
        int h[3];
        for (int i = 0; i < 3; i++) { h[i] = hexNibble(s[i]); if (h[i] < 0) return 0; }
        r = h[0] * 17; g = h[1] * 17; b = h[2] * 17;
    } else {
        return 0;
    }
    out->r = (Uint8)r; out->g = (Uint8)g; out->b = (Uint8)b;
    return 1;
}

static void themeParseFile(const char* path) {
    FILE* f = fopen(path, "r");
    if (!f) return;
    char line[256];
    while (fgets(line, sizeof(line), f)) {
        ...
        *keyEnd = '\0';
        ThemeRGB color;
        if (!parseHexColor(eq + 1, &color)) continue;
        if ((size_t)(keyEnd - key) > 31) continue; // absurdly long key
        if (themeKeyCount < THEME_MAX_KEYS) {
            snprintf(themeKeys[themeKeyCount], sizeof(themeKeys[themeKeyCount]), "%.31s", key);
            themeVals[themeKeyCount] = color;
            themeKeyCount++;
        }
    }
    fclose(f);
...
static int themeFind(const char* key, ThemeRGB* out) {
    for (int i = 0; i < themeKeyCount; i++) {
        if (strcmp(themeKeys[i], key) == 0) { *out = themeVals[i]; return 1; }
    }
    return 0;
}

static ThemeRGB themeMix(ThemeRGB a, ThemeRGB b, float t) {
    ThemeRGB out;
    out.r = (Uint8)(a.r + (b.r - a.r) * t + 0.5f);
    out.g = (Uint8)(a.g + (b.g - a.g) * t + 0.5f);
    out.b = (Uint8)(a.b + (b.b - a.b) * t + 0.5f);
    return out;
}

static void loadTheme(void) {
    char path[512];
    themeKeyCount = 0; // clear stale keys so deleted files truly revert
    themeSnow = (ThemeRGB){ 0xff, 0xff, 0xff };
    themeInk = (ThemeRGB){ 0, 0, 0 };
    themeRed = (ThemeRGB){ 0xff, 0, 0 };
    themeGreen = (ThemeRGB){ 0, 0xbf, 0 };
    themeBlue = (ThemeRGB){ 0, 0, 0xff };
    themeYellow = (ThemeRGB){ 0xff, 0xff, 0 };
    themeMagenta = (ThemeRGB){ 0xff, 0, 0xff };
    themeCyan = (ThemeRGB){ 0, 0xff, 0xff };
    themeStone = (ThemeRGB){ 0x80, 0x80, 0x80 };
    themeMoss = (ThemeRGB){ 0xbf, 0xbf, 0 };
    themeBump = (ThemeRGB){ 0xc0, 0xc0, 0xc0 };
    themeConfigPath(path, sizeof(path));
    themeParseFile(path);
    if (themeKeyCount == 0) return; // no theme file: classic defaults stand

    ThemeRGB v;
    if (themeFind("snow", &v) || themeFind("background", &v)) themeSnow = v;
    if (themeFind("ink", &v) || themeFind("foreground", &v) || themeFind("bright_foreground", &v)) themeInk = v;

    ThemeRGB accent = { 0, 0, 0 };
    int haveAccent = themeFind("accent", &accent);
    ThemeRGB classic;
#define THEME_ROLE(key, field, cr, cg, cb) \
    do { \
        if (!themeFind(key, &field)) { \
            if (haveAccent) field = accent; \
            else { classic.r = (cr); classic.g = (cg); classic.b = (cb); field = classic; } \
        } \
    } while (0)
    THEME_ROLE("red", themeRed, 0xff, 0, 0);
    THEME_ROLE("green", themeGreen, 0, 0xbf, 0);
    THEME_ROLE("blue", themeBlue, 0, 0, 0xff);
    THEME_ROLE("yellow", themeYellow, 0xff, 0xff, 0);
    THEME_ROLE("magenta", themeMagenta, 0xff, 0, 0xff);
    THEME_ROLE("cyan", themeCyan, 0, 0xff, 0xff);
#undef THEME_ROLE

    themeStone = themeMix(themeSnow, themeInk, 0.5f);
    themeMoss = themeMix(themeGreen, themeInk, 0.3f);
    themeBump = themeMix(themeSnow, themeInk, 0.45f);
    recordThemeMtimes();
}

// Classic 16-color VGA sprite palette -> theme roles. Returns 1 if mapped.
// keepWhite skips the white->snow rule (the white-furred yeti, resource IDs
// 68-81, would vanish into dark snow otherwise).
static int themeMapRGB(Uint8 r, Uint8 g, Uint8 b, int keepWhite, ThemeRGB* out) {
    if (r == 0 && g == 0 && b == 0) *out = themeInk;
    else if (r == 0xff && g == 0xff && b == 0xff) { if (!keepWhite) *out = themeSnow; else return 0; }
    else if (r == 0xc0 && g == 0xc0 && b == 0xc0) *out = themeBump;
    else if (r == 0x80 && g == 0x80 && b == 0x80) *out = themeStone;
    else if ((r == 0 && g == 0xbf && b == 0) || (r == 0 && g == 0xff && b == 0)) *out = themeGreen;
    else if (r == 0xbf && g == 0xbf && b == 0) *out = themeMoss;
    else if (r == 0x80 && g == 0x80 && b == 0) *out = themeMoss;
    else if ((r == 0xbf && g == 0 && b == 0) || (r == 0xff && g == 0 && b == 0)) *out = themeRed;
    else if (r == 0x80 && g == 0 && b == 0) *out = themeRed;
    else if ((r == 0 && g == 0 && b == 0xbf) || (r == 0 && g == 0 && b == 0xff)) *out = themeBlue;
    else if (r == 0 && g == 0 && b == 0x80) *out = themeBlue;
    else if ((r == 0xff && g == 0 && b == 0xff) || (r == 0xbf && g == 0 && b == 0xbf)) *out = themeMagenta;
    else if (r == 0xff && g == 0xff && b == 0) *out = themeYellow;
    else if (r == 0 && g == 0xff && b == 0xff) *out = themeCyan;
    else return 0;
    return 1;
}

// Rewrite a palettized sprite surface's palette in place. Called right after
// IMG_Load, before the atlas blit, so every sprite picks up the theme.
static void applyPaletteTheme(SDL_Surface* bitmap, int keepWhite) {
    if (!bitmap || !bitmap->format || !bitmap->format->palette) return;
    SDL_Palette* pal = bitmap->format->palette;
    SDL_Color colors[256];
    int n = pal->ncolors < 256 ? pal->ncolors : 256;
    for (int i = 0; i < n; i++) {
        ThemeRGB mapped;
        if (themeMapRGB(pal->colors[i].r, pal->colors[i].g, pal->colors[i].b, keepWhite, &mapped)) {
            colors[i].r = mapped.r; colors[i].g = mapped.g; colors[i].b = mapped.b;
        } else {
            colors[i] = pal->colors[i];
        }
        colors[i].a = 0xff;
    }
    SDL_SetPaletteColors(pal, colors, 0, n);
}

// Resolved path of the game's own theme file. $XDG_CONFIG_HOME wins,
// ~/.config is the fallback. No desktop-environment paths here by design.
static void themeConfigPath(char* out, size_t n) {
    ...
    if (strlen(base) + sizeof("/skifree/theme.toml") < n) {
        snprintf(out, n, "%s/skifree/theme.toml", base);
    } else {
        out[0] = '\0'; // absurdly long env: no theme file, classic look
    }
}

// mtime of the game's own theme file in nanoseconds, or 0 when missing,
// so appearing/disappearing files also trigger a reload.
static long long themeWatchedMtimeNs = 0;

static long long themeCurrentMtimeNs(void) {
    char path[512];
    struct stat st;
    themeConfigPath(path, sizeof(path));
...
static void recordThemeMtimes(void) {
    themeWatchedMtimeNs = themeCurrentMtimeNs();
}

static int themeFileChanged(void) {
    return themeCurrentMtimeNs() != themeWatchedMtimeNs;
}

// Rebuild both sprite atlases from the current theme roles. Geometry is
// deterministic (same embedded resources), so all live actors, sprite rects
// and sheet pointers stay valid; only pixel colors change.
static void rebuildThemeAtlases(void) {
    if (largeTextureAtlas) { SDL_DestroyTexture(largeTextureAtlas); largeTextureAtlas = NULL; }
    if (smallTextureAtlas) { SDL_DestroyTexture(smallTextureAtlas); smallTextureAtlas = NULL; }
    if (largeBitmapDC) { SDL_FreeSurface(largeBitmapDC); largeBitmapDC = NULL; }
    if (smallBitmapDC) { SDL_FreeSurface(smallBitmapDC); smallBitmapDC = NULL; }
    createBitmapSheets(mainWindowDC);
}

// Check for a theme switch (called ~1/sec from the main loop). Snow follows
// every frame via mainWindowPaint; sprites and status bar refresh here.
void paintStatusWindow(HWND hWnd);
static void pollThemeLive(void) {
    if (!themeFileChanged()) return;
    loadTheme();
    recordThemeMtimes();
    rebuildThemeAtlases();
...
int main(int argc, char* argv[]) {
    ...
    while (is_running) {
        ...
        if (isGameTimerRunning && last_timer + 40 < SDL_GetTicks()) {
            ...
            if (++themePollTick >= 30) { // ~1.2s: live theme switch
                themePollTick = 0;
                pollThemeLive();
            }
        }
...
void drawText(HDC hdc, LPCSTR textStr, short x, short* y, int textLen) {
    ...
    SDL_Surface* surface;
    SDL_Color textColor = { themeInk.r, themeInk.g, themeInk.b, 0 };
...
int initWindows() {
    ...
    SDL_SetRenderDrawColor(renderer, themeSnow.r, themeSnow.g, themeSnow.b, 0xFF);
...
void paintStatusWindow(HWND hWnd) {
    ...
    SDL_FillRect(statusWindowSurface, NULL, SDL_MapRGB(statusWindowSurface->format, themeInk.r, themeInk.g, themeInk.b));
    SDL_Rect r = { 1, 1, statusWindowSurface->w - 2, statusWindowSurface->h - 2 };
    SDL_FillRect(statusWindowSurface, &r, SDL_MapRGB(statusWindowSurface->format, themeSnow.r, themeSnow.g, themeSnow.b));
...
HBITMAP loadBitmapResource(uint32_t resourceId) {
    ...
    applyPaletteTheme(bitmap, resourceId >= 68 && resourceId <= 81);
    return bitmap;
}
...
void formatAndPrintStatusStrings(HDC windowDC) {
    ...
    SDL_FillRect(statusWindowSurface, &rect, SDL_MapRGB(statusWindowSurface->format, themeSnow.r, themeSnow.g, themeSnow.b));
